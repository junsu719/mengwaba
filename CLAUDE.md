...
- **2026-08-28 起補充**:行政區頁(`/trash/{city}/{district}/`)已改回完全靜態預生成(build 時讀 `data/normalized/*.json` 窮舉,不再 on-demand 查 D1,見 DECISIONS.md 2026-08-28 條目),故季度資料更新流程**推送 D1 之後多一個必要步驟**:重新 `npm run build` + 部署,行政區頁才會反映新資料——只推 D1 不重新 build+deploy,行政區頁會安靜停留在舊資料且無任何錯誤訊息或異常狀態碼可供察覺。清運點頁(`/trash/{city}/{district}/{point}/`)仍維持 on-demand 查 D1,不受此限制,下次請求即自動反映新資料。
